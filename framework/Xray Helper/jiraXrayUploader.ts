...
export class JiraXrayUploader {
  ...
  private async flushBatch(): Promise<void> {
    ...
    try {
      if (!this.state) {
        ...
        /*
         * Batch 2 onwards uses the normal Cucumber import endpoint.
         * uploadToExistingExecution() adds @<executionKey> at the
         * feature level so Xray maps these results into the same
         * Test Execution created by batch 1.
         */
  ...
  private async uploadToExistingExecution(
     ...
     * IMPORTANT:
     *
     * For batches after the first multipart batch, Xray maps the
     * Cucumber results to the existing Test Execution using the
     * execution key as a Cucumber tag, e.g. @CVP-7365.
     *
     * Therefore we intentionally do NOT send ?testExecKey=... here.
     * The endpoint is the plain Cucumber import endpoint and the
     * feature-level tag performs the mapping.
     */
    const endpoint =
      `${this.config.jiraBaseUrl}` +
    ...
    /*
     * Add @<executionKey> to the feature-level tags before the
     * Cucumber JSON is sent to Xray.
     *
     * This modifies only the generated batch file, never the original
     * cucumber-report.json.
     */
    await this.addExecutionKeyTagToBatch(
      batchPath,
      executionKey
    );
  ...
  // ========================================================
  // ADD EXECUTION KEY TAG TO BATCH
  // ========================================================

  private async addExecutionKeyTagToBatch(
    batchPath: string,
    executionKey: string
  ): Promise<void> {
    const executionTag =
    ...
    if (!Array.isArray(batch)) {
      throw new Error(
        `Invalid Cucumber batch file: ${batchPath}. Expected a JSON array.`
      );
    }
     ...
     * written batch that could be picked up by a retry.
     */
    const tempPath =
      `${batchPath}.tagged.tmp`;
    ...
    console.log(
      `Added feature tag ${executionTag} to ${batch.length} feature(s) in ${path.basename(batchPath)}.`
    );
  }
